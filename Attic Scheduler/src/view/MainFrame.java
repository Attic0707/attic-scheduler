	...
	public MainFrame() {
		...
		kanbanPanel.setCardListener(new TaskCardListener() {
			...
			public void editCard(int paneId, int cardNo) {
				kanbanPanel.editCard(paneId, cardNo);
			}
		});
...
//				taskCards.remove(row);
		...
		toolbar.setSignUpListener(new SignUpListener() {
			public void signUpEventOccured(SignUpEvent sue) {
				...
				formPanel.getUserNameList(controller.getUserNameList());
			...
			public void logInEventOccured(LogInEvent lie) {
				if (controller.checkForValid(lie.getUserName(), lie.getPassword()) == true) {
					...
					for (User u : controller.getUserList()) {
						if ((u.getUserName()).equals(lie.getUserName())) {
							profilePanel.userInfoDetails(true, u.getFirstName(), u.getLastName(), u.getUserName(), u.getPassword(), u.getStreet(), u.getPostalCode(), u.getCity(),
									u.getState(), u.getCountry(), u.getBio(), u.getEmail(), u.getCompany(),
									u.getWebsite(), u.getSocial(), u.getPhone(), u.getMemberSince());
						}
					}
...
//					taskCards.placeCard(fv.getIssueId(), fv.getIssueName(), fv.getDate());
				}
